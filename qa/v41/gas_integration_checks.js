...
await test('Current summaries use Smart Sync, explicit QC status, no setup or automatic notifications',async()=>{
  ...
  assert('qc'in g.get({action:'status'}).data);assert.equal(g.get({action:'capabilities'}).data.backendVersion,'4.1.0');assert.equal(g.get({action:'auditStatus'}).data.disabled,true);assert.equal(g.ctx.maybeProdPendingReminder_(),false);
