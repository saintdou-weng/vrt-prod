...
(function(root){
  ...
  function parsePartsWorkbook(wb,filename){
    const out={parts:[],txns:[],stockReports:[],needleChanges:[],invoiceSummaries:[],warnings:[],source:filename};
    for(const name of wb.SheetNames){const a=rows(wb,name),top=a.slice(0,4).flat().join(' ');
      if(/Needles Stock Report|Spareparts Stock Report/i.test(top)){
      ...
      }else if(header(a,['Invoice No','Subtotal (USD)'])>=0){
        const h=header(a,['Invoice No','Subtotal (USD)']),head=a[h];for(let i=h+1;i<a.length;i++){const get=t=>a[i][column(head,[t])],date=ymd(get('Date')),invoiceNo=String(get('Invoice No')||'').trim(),amount=number(get('Subtotal (USD)'));if(!date||!invoiceNo||amount==null)continue;const r={date,invoiceNo,amount,supplier:String(get('Supplier')||''),statementMonth:String(get('Month')||date.slice(0,7)),notes:String(get('Notes')||''),sourceFile:filename,sourceSheet:name,sourceRow:i+1};r.id='INVS-'+hash([date,invoiceNo,norm(r.supplier)]);out.invoiceSummaries.push(r)}
      }else if(norm(name)==='TRANSACTION DETAIL'||header(a,['Invoice No','Part No','Calculated Amount'])>=0||header(a,['Invoice No','Part No','Unit Price','Amount'])>=0){
        ...
        for(let i=h+1;i<a.length;i++){const r=a[i],partNo=String(r[col(['Part No'])]||'').trim();if(!partNo)continue;
          const v=t=>r[col(t)],tx={type:String(head.some(x=>norm(x)==='TYPE')?v(['Type'])||'PURCHASE':'PURCHASE'),partNo,name:String(v(['Standard Name'])||v(['Raw Item Name'])||partNo),spec:String(v(['Spec'])||''),category:String(v(['Category'])||'Other'),brand:String(v(['Brand'])||''),machineType:String(v(['Machine / Use'])||''),qty:number(v(['Qty']))||0,unit:String(v(['Unit'])||''),unitPrice:number(v(['Unit Price']))||0,amount:number(v(['Calculated Amount']))??number(v(['Source Amount']))??number(v(['Amount (USD)'])),invoiceNo:String(v(['Invoice No'])||''),invoiceDate:ymd(v(['Date'])),statementMonth:String(v(['Statement Month'])||''),supplier:String(v(['Supplier'])||''),sourceFile:String(v(['Source File'])||filename),sourceSheet:String(v(['Source Sheet'])||name),sourceRow:number(v(['Source Row']))||i+1,includeInMaster:norm(v(['Include in Master']))!=='NO',remark:String(v(['Notes'])||''),raw:copy(r)};
          tx.date=tx.invoiceDate;if(!tx.statementMonth)tx.statementMonth=String(tx.date||'').slice(0,7);tx.id=String(v(['Record ID'])||'PUR-'+hash([tx.sourceFile,tx.invoiceNo,tx.partNo,i+1]));out.txns.push(tx);
        }
    ...
    if(out.invoiceSummaries.length)out.warnings.push('2024 發票摘要獨立保留供核對；可辨識採購明細只是部分資料，不與摘要重複加總。');
    if(!out.invoiceSummaries.length&&!out.parts.length&&!out.txns.length&&!out.stockReports.length&&!out.needleChanges.length)throw new Error('未辨識有效資料。支援價格基準、零件庫存、機針庫存、Needle Change、Inventory / CSV。');return out;
  }
  ...
  function partUnit(v){const u=norm(v).replace(/\.$/,'');for(const [key,terms] of Object.entries({PCS:['PCS','PC','PIECE','PIECES','個','顆','件','支','根'],SET:['SET','SETS','套','組'],PACK:['PACK','PACKS','PK','包'],BOX:['BOX','BOXES','盒'],CARTON:['CARTON','CTN','箱'],BOTTLE:['BOTTLE','BTL','瓶'],CAN:['CAN','CANS','罐'],DOZEN:['DOZEN','DOZ','打'],MACHINE:['台'],METER:['M','METER','METRE','公尺','米'],YARD:['YD','YDS','YARD','YARDS','碼']}))if(terms.includes(u))return key;return u}
  root.VRTData39={partUnit,copy,norm,number,stable,hash,ymd,family,revision,initIE,insertIE,currentIE,eligibleIE,mergeIE,officialRows,resolveOfficial,officialByStyle,snapshotFingerprint,parsePartsWorkbook,mergeReports};
