...
(function(g){
  ...
  function install(opts){
    ...
    async function run(reason,extra){
      ...
      st.busy=true;st.lastRun=Date.now();status('pull','自動檢查更新…','busy');
      let pullOK=true,pushOK=true;
      try{
        if(typeof st.opts.pull==='function')try{const r=await st.opts.pull({silent:true,auto:true,reason});pullOK=(r!==false)}catch(e){pullOK=false;console.warn('[VRT Auto pull '+key+']',e)}
        if(typeof st.opts.push==='function')try{status('push','自動比對待上傳…','busy');const r=await st.opts.push(Object.assign({silent:true,auto:true,reason},extra));pushOK=(r!==false)}catch(e){pushOK=false;console.warn('[VRT Auto push '+key+']',e)}
      ...
      finally{st.busy=false;lockRelease(st.lockKey);if(st.queued){const q=st.queued;st.queued=null;schedule(q.reason,q.extra,350)}}
    }
    ...
    st.run=run;st.schedule=schedule;
  ...
  g.VRTProdAutoSync={version:VERSION,install,schedule:(k,r,e,d)=>C[k]?C[k].schedule(r,e,d):false,markDirty:(k,r,e,d)=>C[k]?C[k].schedule(r||'change',e,d):false,run:(k,r,e)=>C[k]?C[k].run(r,e):Promise.resolve(false),flush:(k,r,e)=>C[k]?C[k].run(r,e):Promise.resolve(false),pending:k=>read(k),state:k=>C[k]||null};
})(window);
